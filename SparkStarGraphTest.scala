...
object SparkStarGraphTest{  
    def main(args:Array[String]){  
        val conf = new SparkConf().setAppName("Simple Application")  
        val sc = new SparkContext(conf)

	val queryArr: Array[(String, String)] = Array(("<http://db.uwaterloo.ca/~galuc/wsdbm/hasGenre>", "<http://db.uwaterloo.ca/~galuc/wsdbm/SubGenre2>"), ("<http://purl.org/ontology/mo/conductor>", "?y1"), ("<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>", "?y3"))

		val edges: RDD[Edge[String]] = sc.textFile("/pengpeng/input/WatDiv/watdiv1M_graph.txt").map { line => 
			val fields = line.split("\t") 
			Edge(fields(0).toLong, fields(2).toLong, fields(1))
		}
  
        val users = sc.textFile("/pengpeng/input/WatDiv/watdiv1M_uris.txt").map { line =>  
            val fields = line.split(",")  
